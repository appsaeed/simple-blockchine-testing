const $ = {
  ...
  addBlock: async function (data) {
    ...
    const block = {
      id: id,
      data: data,
      prev_has: await $.previousHash(),
      datetime: Date.now(),
    };
    await this.hashBlock(block);
  },
  previousHash: async function () {
    let index = this.blocks.length - 1;
    return this.blocks[index].hash;
  },
  ...
  hashBlock: async function (block) {
    ...
    async function recur(block) {
      ...
      const hash = await th.hash(data);
      if (await th.verify(hash)) {
        th.blocks.push({
          block: block,
          hash: hash,
        });
